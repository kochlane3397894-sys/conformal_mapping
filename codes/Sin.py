...
def Sin(z, t):
    """参数化的线性分式变换：从恒等映射 (t=0) 过渡到 Sin 变换 (t=1)"""
#    return  (np.exp( 1j * z ) - np.exp( -1j * z ) )/ (2 * 1j)
    return z * (1-t) + t * np.sin(z)

# ========== 1. 生成原始网格（上半平面） ==========
# 水平线：不同虚部 y0 的直线，x 从 -2 到 2
x_vals = np.linspace(-np.pi / 2, np.pi / 2, 60)          # 每条水平线上的 x 坐标
y_levels = np.linspace(0, 10, 50)       # 水平线的虚部（y0 > 0）

# 垂直线：不同实部 x0 的直线，y 从 0 到 2
y_vals = np.linspace(0, 10, 50)           # 每条垂直线上的 y 坐标
x_levels = np.linspace(-np.pi / 2, np.pi / 2, 60)        # 垂直线的实部（x0）
...
# ========== 5. 保存为 HTML ==========
fig.write_html('Sin_animation.html', auto_open=False)
