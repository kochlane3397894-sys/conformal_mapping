...
def HalfPlane_HalfDisk(z, t):
    """参数化的线性分式变换：从恒等映射 (t=0) 过渡到 HalfPlane_HalfDisk 变换 (t=1)"""
    #return - z * (2 * t -1) - t * np.sqrt(z**2 - 1)
    return 0.5 * (z + np.sqrt(z**2 - 1))
...
for t in t_values:
    ...
    for y0 in y_levels:
        # 该水平线的原始复数点
        z_line = x_vals + 1j * y0
        w = HalfPlane_HalfDisk(z_line, t)             # 变换后的复数点
...
# ========== 5. 保存为 HTML ==========
fig.write_html('HalfPlane_HalfDisk_animation.html', auto_open=False)
