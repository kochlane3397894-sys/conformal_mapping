...
def HalfDisk_firstQuadrant(z, t):
    """参数化的线性分式变换：从恒等映射 (t=0) 过渡到 HalfDisk_firstQuadrant 变换 (t=1)"""
    return (z - t) / (t * z + 1)

# ========== 1. 生成原始网格（上半平面） ==========
# 水平线：不同虚部 y0 的直线，x 从 -2 到 2
x_vals = np.linspace(0, 10, 50)          # 每条水平线上的 x 坐标
y_levels = np.linspace(0, 10, 50)       # 水平线的虚部（y0 > 0）

# 垂直线：不同实部 x0 的直线，y 从 0 到 2
y_vals = np.linspace(0, 10, 50)           # 每条垂直线上的 y 坐标
x_levels = np.linspace(0, 10, 50)        # 垂直线的实部（x0）
...
for t in t_values:
    ...
    for y0 in y_levels:
        # 该水平线的原始复数点
        z_line = x_vals + 1j * y0
        w = HalfDisk_firstQuadrant(z_line, t)             # 变换后的复数点
...
for t in t_values:
    ...
    for x0 in x_levels:
        z_line = x0 + 1j * y_vals
        w = HalfDisk_firstQuadrant(z_line, t)
        lines_t.append((w.real, w.imag))
...
# ========== 5. 保存为 HTML ==========
fig.write_html('HalfDisk_firstQuadrant_animation.html', auto_open=False)
